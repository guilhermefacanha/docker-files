#!/usr/bin/env bash
# DSF Hub onboarding: Azure Database for MariaDB (Single Server) via Event Hubs
# Pipeline: MariaDB → Diagnostic Settings → Azure Monitor → Event Hubs → DSF Agentless Gateway

set -e
. "$(dirname "${BASH_SOURCE[0]}")/00-env.sh"

SERVER_NAME="${DB_SERVER_NAME:-mymariadb${ENV_SUFFIX:-}-dsf}"
DB_VERSION="10.3"
CONTAINER_NAME="floci-az-mariadb-${SERVER_NAME}"
...
DIAG_PATH="${ARM_BASE}/providers/Microsoft.DBforMariaDB/servers/${SERVER_NAME}/providers/microsoft.insights/diagnosticSettings/dsf-audit"
...
wait_ready() {
    ...
    while [ "$tries" -lt 40 ]; do
        state=$(az_get "${ARM_BASE}/providers/Microsoft.DBforMariaDB/servers/${name}?api-version=2018-06-01" \
                | python3 -c "import sys,json; d=json.load(sys.stdin); print(d.get('properties',{}).get('userVisibleState',''))" 2>/dev/null || echo "")
        if [ "$state" = "Ready" ]; then
            if docker exec "${CONTAINER_NAME}" mariadb-admin -u root -p"${DB_MASTER_PASS}" ping --silent 2>/dev/null || \
               docker exec "${CONTAINER_NAME}" mysqladmin -u root -p"${DB_MASTER_PASS}" ping --silent 2>/dev/null; then
                return 0
            fi
        fi
...
# ── STEP 2: Create MariaDB Single Server ──────────────────────────────────────
step "STEP 2: Creating Azure Database for MariaDB '${SERVER_NAME}'"

existing=$(az_get "${ARM_BASE}/providers/Microsoft.DBforMariaDB/servers/${SERVER_NAME}?api-version=2018-06-01" 2>/dev/null \
           | python3 -c "import sys,json; d=json.load(sys.stdin); print(d.get('properties',{}).get('userVisibleState',''))" 2>/dev/null || echo "")

if [ -n "$existing" ] && [ "$existing" != "" ]; then
    info "Server already exists (state: ${existing}) — skipping creation."
else
    az_put "${ARM_BASE}/providers/Microsoft.DBforMariaDB/servers/${SERVER_NAME}?api-version=2018-06-01" \
        -d "{
...
RESP=$(az_get "${ARM_BASE}/providers/Microsoft.DBforMariaDB/servers/${SERVER_NAME}?api-version=2018-06-01")
...
# ── STEP 2b: Start host-accessible proxy ─────────────────────────────────────
step "STEP 2b: Starting host port proxy"

PROXY_CONTAINER="azdb-proxy-mariadb-${SERVER_NAME}"
...
# ── STEP 3: Create users ───────────────────────────────────────────────────────
step "STEP 3: Creating admin and audit management users"

db_exec() {
    docker exec "${CONTAINER_NAME}" mariadb -u root -p"${DB_MASTER_PASS}" -e "$1" 2>/dev/null || \
    docker exec "${CONTAINER_NAME}" mysql   -u root -p"${DB_MASTER_PASS}" -e "$1" 2>/dev/null
}
...
"
info "Users ready. General query log enabled."
...
info "Diagnostic settings: MariaDB audit logs → ${AZ_EVENTHUB_NAMESPACE}/${AZ_EVENTHUB_NAME}"

# ── Summary ───────────────────────────────────────────────────────────────────
step "Setup complete — DSF Hub asset hierarchy for Azure Database for MariaDB"
cat <<EOF
  ...
  Azure Subscription asset
    ...
    └── Resource Group: ${AZ_RESOURCE_GROUP}
        ...
        └── Azure Database for MariaDB (Single Server)
