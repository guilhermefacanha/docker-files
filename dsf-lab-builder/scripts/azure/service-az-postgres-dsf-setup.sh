#!/usr/bin/env bash
# DSF Hub onboarding: Azure Database for PostgreSQL (Flexible Server) via Event Hubs
# Pipeline: PostgreSQL → Diagnostic Settings → Azure Monitor → Event Hubs → DSF Agentless Gateway

set -e
. "$(dirname "${BASH_SOURCE[0]}")/00-env.sh"

SERVER_NAME="${DB_SERVER_NAME:-mypostgres${ENV_SUFFIX:-}-dsf}"
DB_VERSION="16"
CONTAINER_NAME="floci-az-pg-${SERVER_NAME}"
NETWORK="${FLOCI_AZ_NETWORK:-floci-az1_default}"

step()  { echo; echo "=== $* ==="; }
info()  { echo "    $*"; }

ARM_BASE="/subscriptions/${AZ_SUBSCRIPTION_ID}/resourceGroups/${AZ_RESOURCE_GROUP}"
DIAG_PATH="${ARM_BASE}/providers/Microsoft.DBforPostgreSQL/flexibleServers/${SERVER_NAME}/providers/microsoft.insights/diagnosticSettings/dsf-audit"
EH_RULE_ID="${ARM_BASE}/providers/Microsoft.EventHub/namespaces/${AZ_EVENTHUB_NAMESPACE}/authorizationRules/RootManageSharedAccessKey"

db_psql() { docker exec -e PGPASSWORD="${DB_MASTER_PASS}" "${CONTAINER_NAME}" psql -U "${DB_MASTER_USER}" -d postgres "$@"; }

wait_ready() {
    local name="$1" tries=0
    info "Waiting for server '${CONTAINER_NAME}' to become Ready..."
    while [ "$tries" -lt 40 ]; do
        state=$(az_get "${ARM_BASE}/providers/Microsoft.DBforPostgreSQL/flexibleServers/${name}?api-version=2023-06-01-preview" \
                | python3 -c "import sys,json; d=json.load(sys.stdin); print(d.get('properties',{}).get('state',''))" 2>/dev/null || echo "")
        if [ "$state" = "Ready" ]; then
            if docker exec -e PGPASSWORD="${DB_MASTER_PASS}" "${CONTAINER_NAME}" psql -U "${DB_MASTER_USER}" -d postgres -c "SELECT 1" >/dev/null 2>&1; then
                return 0
            fi
        fi
        tries=$((tries + 1)); sleep 3
    done
    echo "ERROR: server '${name}' did not become Ready (state: ${state})" >&2; exit 1
}

# ── STEP 1: Create Event Hub namespace via floci-az native API (starts Artemis) ──
step "STEP 1: Creating Event Hub namespace '${AZ_EVENTHUB_NAMESPACE}' (Artemis AMQP)"

EH_ACCOUNT_PREFIX="${AZ_EVENTHUB_ACCOUNT_PREFIX:-dsf-lab}"
EH_NS_URL="${FLOCI_AZ_ENDPOINT}/${EH_ACCOUNT_PREFIX}-eventhub/namespaces/${AZ_EVENTHUB_NAMESPACE}"
EH_RESP=$(curl -sf -X PUT -H "Content-Type: application/json" "${EH_NS_URL}" \
    -d "{\"entities\":\"${AZ_EVENTHUB_NAME}\",\"consumerGroups\":\"\$Default\"}" 2>/dev/null || echo '{"mocked":true}')
EH_MOCKED=$(echo "$EH_RESP" | python3 -c "import sys,json; d=json.load(sys.stdin); print(str(d.get('mocked',True)).lower())" 2>/dev/null || echo "true")

if [ "$EH_MOCKED" = "false" ]; then
    info "Artemis broker starting for namespace '${AZ_EVENTHUB_NAMESPACE}'..."
    for i in $(seq 1 30); do
        AMQP_PORT=$(curl -sf "${EH_NS_URL}" 2>/dev/null | python3 -c "import sys,json; d=json.load(sys.stdin); print(d.get('amqpPort',0))" 2>/dev/null || echo "0")
        [ "${AMQP_PORT:-0}" -gt 0 ] && { info "Artemis ready (AMQP port ${AMQP_PORT})"; break; }
        sleep 3
    done
else
    info "Event Hub in mocked mode — AMQP disabled."
fi
info "Event Hub ready: ${AZ_EVENTHUB_NAMESPACE}/${AZ_EVENTHUB_NAME}"

# ── STEP 2: Create PostgreSQL Flexible Server ─────────────────────────────────
step "STEP 2: Creating Azure Database for PostgreSQL '${SERVER_NAME}'"

existing=$(az_get "${ARM_BASE}/providers/Microsoft.DBforPostgreSQL/flexibleServers/${SERVER_NAME}?api-version=2023-06-01-preview" 2>/dev/null \
           | python3 -c "import sys,json; d=json.load(sys.stdin); print(d.get('properties',{}).get('state',''))" 2>/dev/null || echo "")

if [ -n "$existing" ] && [ "$existing" != "" ]; then
    info "Server already exists (state: ${existing}) — skipping creation."
else
    az_put "${ARM_BASE}/providers/Microsoft.DBforPostgreSQL/flexibleServers/${SERVER_NAME}?api-version=2023-06-01-preview" \
        -d "{
          \"location\": \"${AZ_LOCATION}\",
          \"sku\": {\"name\": \"Standard_B1ms\", \"tier\": \"Burstable\"},
          \"properties\": {
            \"administratorLogin\": \"${DB_MASTER_USER}\",
            \"administratorLoginPassword\": \"${DB_MASTER_PASS}\",
            \"version\": \"${DB_VERSION}\",
            \"storage\": {\"storageSizeGB\": 32}
          }
        }" > /dev/null
    info "Server creation requested."
fi

wait_ready "$SERVER_NAME"

RESP=$(az_get "${ARM_BASE}/providers/Microsoft.DBforPostgreSQL/flexibleServers/${SERVER_NAME}?api-version=2023-06-01-preview")
DB_IP=$(echo "$RESP" | python3 -c "import sys,json; d=json.load(sys.stdin); print(d.get('properties',{}).get('fullyQualifiedDomainName',''))" 2>/dev/null || echo "")
DB_PORT=5432

# ── STEP 2b: Start host proxy ─────────────────────────────────────────────────
step "STEP 2b: Starting host port proxy"

PROXY_CONTAINER="azdb-proxy-postgres-${SERVER_NAME}"
docker rm -f "${PROXY_CONTAINER}" 2>/dev/null || true

PROXY_PORT=""
for p in $(seq "$DB_PROXY_BASE_PORT" "$DB_PROXY_MAX_PORT"); do
    if ! docker ps --format "{{.Ports}}" | grep -q ":${p}->"; then
        PROXY_PORT=$p; break
    fi
done

CONTAINER_IP=$(docker inspect "${CONTAINER_NAME}" --format '{{range .NetworkSettings.Networks}}{{.IPAddress}}{{end}}' 2>/dev/null | head -1 || echo "")

if [ -n "$PROXY_PORT" ] && [ -n "$CONTAINER_IP" ]; then
    docker run -d --rm \
        --name "${PROXY_CONTAINER}" \
        --network "${NETWORK}" \
        -p "${PROXY_PORT}:${PROXY_PORT}" \
        alpine/socat \
        TCP-LISTEN:${PROXY_PORT},fork,reuseaddr "TCP:${CONTAINER_IP}:${DB_PORT}" >/dev/null
    info "Proxy started: localhost:${PROXY_PORT} → ${CONTAINER_IP}:${DB_PORT}"
    info "DBeaver: host=localhost  port=${PROXY_PORT}  user=${DB_MASTER_USER}  pass=${DB_MASTER_PASS}"
fi

# ── STEP 3: Configure users and audit logging ─────────────────────────────────
step "STEP 3: Creating admin user and enabling audit logging"

docker exec -e PGPASSWORD="${DB_MASTER_PASS}" "${CONTAINER_NAME}" psql -U "${DB_MASTER_USER}" -d postgres -c "
DO \$\$
BEGIN
  IF NOT EXISTS (SELECT FROM pg_roles WHERE rolname = '${DB_AUDIT_USER}') THEN
    CREATE USER ${DB_AUDIT_USER} WITH PASSWORD '${DB_AUDIT_PASS}';
    GRANT pg_monitor TO ${DB_AUDIT_USER};
  END IF;
END \$\$;
"

docker exec -e PGPASSWORD="${DB_MASTER_PASS}" "${CONTAINER_NAME}" psql -U "${DB_MASTER_USER}" -d postgres \
    -c "ALTER SYSTEM SET log_statement = 'all';" \
    -c "ALTER SYSTEM SET log_connections = 'on';" \
    -c "ALTER SYSTEM SET log_disconnections = 'on';" \
    -c "ALTER SYSTEM SET log_duration = 'on';" \
    -c "SELECT pg_reload_conf();" > /dev/null
info "Audit logging enabled (log_statement=all)."

# ── STEP 4: Configure Diagnostic Settings ────────────────────────────────────
step "STEP 4: Configuring diagnostic settings → Event Hubs"

az_put "${DIAG_PATH}?api-version=2021-05-01-preview" \
    -d "{
      \"properties\": {
        \"eventHubAuthorizationRuleId\": \"${EH_RULE_ID}\",
        \"eventHubName\": \"${AZ_EVENTHUB_NAME}\",
        \"logs\": [
          {\"category\": \"PostgreSQLLogs\", \"enabled\": true},
          {\"category\": \"QueryStoreRuntimeStatistics\", \"enabled\": false},
          {\"category\": \"QueryStoreWaitStatistics\", \"enabled\": false}
        ],
        \"metrics\": [{\"category\": \"AllMetrics\", \"enabled\": false}]
      }
    }" > /dev/null
info "Diagnostic settings: PostgreSQL logs → ${AZ_EVENTHUB_NAMESPACE}/${AZ_EVENTHUB_NAME}"

# ── Summary ───────────────────────────────────────────────────────────────────
step "Setup complete — DSF Hub asset hierarchy for Azure Database for PostgreSQL"
cat <<EOF

  Azure Subscription asset
    Subscription  : ${AZ_SUBSCRIPTION_ID}

    └── Resource Group: ${AZ_RESOURCE_GROUP}

        └── Azure Database for PostgreSQL (Flexible Server)
              Server Name  : ${SERVER_NAME}
              Version      : ${DB_VERSION}
              Location     : ${AZ_LOCATION}
              FQDN         : ${DB_IP}:${DB_PORT}
              Host Proxy   : localhost:${PROXY_PORT:-N/A}
              Container    : ${CONTAINER_NAME}
              Master User  : ${DB_MASTER_USER} / ${DB_MASTER_PASS}
              Audit User   : ${DB_AUDIT_USER} / ${DB_AUDIT_PASS}

              └── Diagnostic Settings: dsf-audit
                    Log Category  : PostgreSQLLogs
                    Routed to     : ${AZ_EVENTHUB_NAMESPACE}/${AZ_EVENTHUB_NAME}
EOF
echo
